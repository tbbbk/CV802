...
class Dataset:
    def __init__(self, conf):
        ...
        for pattern in image_patterns:
            self.images_lis.extend(glob(os.path.join(image_dir, pattern)))
        self.images_lis = sorted(self.images_lis)
        if len(self.images_lis) == 0:
            raise FileNotFoundError(
                f"No images found under '{image_dir}'. Expected extensions: {', '.join(image_patterns)}")
        ...
        self.pts = torch.from_numpy(np.load(pts_dir)).float().cuda()
        ...
        def _name_to_index(s: str) -> int:
            bn = os.path.basename(s)
            m = re.search(r'(\d+)(?=\.[^.]+$)', bn)
            if m is None:
                m = re.search(r'(\d+)', bn)
            if m is None:
                raise ValueError(f"Cannot parse index from '{s}'")
            return int(m.group(1))

        self.src_idx = []
        for p in pairs:
            splitted = p.split()[1:]  # drop the first one since it is the ref img
            self.src_idx.append(torch.tensor(list(map(_name_to_index, splitted))))
            print(len(self.src_idx[-1]))
    ...
    def gen_random_rays_at(self, img_idx, batch_size):
        ...
        return torch.cat([rays_o.cpu(), rays_v.cpu(), color, mask[:, :1]], dim=-1).cuda(), intrinsics_pair, intrinsics_inv_pair, poses_pair, images_gray_pair    # batch_size, 10
